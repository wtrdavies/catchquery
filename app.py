...
def check_password():
    ...
    # Check if authentication is disabled for local testing
    try:
        disable_auth = st.secrets.get("DISABLE_AUTH_FOR_LOCAL_TESTING", False)
        if disable_auth:
            return True  # Skip authentication for local development
    except Exception:
        pass  # Secrets not available, continue with normal auth
    ...
    def password_entered():
        ...
        if not correct_password:
            st.error("APP_PASSWORD not configured. Please set it in Streamlit secrets or .env file.")
            return False

        if st.session_state["password"] == correct_password:
            st.session_state["password_correct"] = True
            del st.session_state["password"]  # Don't store the password in session
        else:
